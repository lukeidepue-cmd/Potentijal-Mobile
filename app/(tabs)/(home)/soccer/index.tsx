...
const SAMPLE_GOALS: Goal[] = [
  { id: "g1", name: "Passing Reps", done: 120, total: 400 },
  { id: "g2", name: "Dribbling Reps", done: 70, total: 300 },
  { id: "g3", name: "Shots", done: 50, total: 200 },
];
...
export default function SoccerHome() {
  ...
  const onMomentumEnd = (e: NativeSyntheticEvent<NativeScrollEvent>) => setPage(Math.round(e.nativeEvent.contentOffset.x / SNAP));

  const [interval, setInterval] = useState<"7" | "30" | "90" | "180">("7");
  const [query, setQuery] = useState("");
  const totals = useMemo(() => ({ reps: 807, workouts: 9, growthFrom: 12, growthTo: 18 }), [interval, query]);
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.colors.bg0 }} contentContainerStyle={{ padding: theme.layout.xl, gap: theme.layout.lg, paddingBottom: 20 }} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.progressWrap}>
        ...
        <TextInput placeholder="Type a Drill or Exercise…" placeholderTextColor={theme.colors.textLo} value={query} onChangeText={setQuery} style={styles.search} />
      ...
      <View style={{ gap: 20, marginTop: 12 }}>
        <View style={styles.bigCard}>
          <Image source={require("../../../../assets/players/yamal.jpg")} style={styles.heroImageTop} resizeMode="cover" />
          <View style={styles.bigContent}>
            <Text style={styles.bigTitle}>Log Game</Text>
            <Text style={styles.bigSub}>For Optimized Training</Text>
            <Pressable style={{ alignSelf: "flex-start", marginTop: 12 }} onPress={() => router.push("/(tabs)/(home)/soccer/add-game")}>
        ...
        <View style={styles.bigCard}>
          <Image source={require("../../../../assets/players/mbappe.webp")} style={styles.heroImageTop} resizeMode="cover" />
          <View style={styles.bigContent}>
            <Text style={styles.bigTitle}>Log Team Practices</Text>
            <Text style={styles.bigSub}>To Add Additional Training</Text>
            <Pressable style={{ alignSelf: "flex-start", marginTop: 12 }} onPress={() => router.push("/(tabs)/(home)/soccer/add-practice")}>
              <LinearGradient colors={[theme.colors.primary600, "#3BAA6F"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.cta}>
